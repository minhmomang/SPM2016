...
$(function() {
	...
	function get_munti_selected() {
		...
		if (rows.length ==1 ) {
			...
			showdialog('dialogmanual', 0, 'Vui lòng chỉ chọn 1 sản phẩm', '', '');
			return;
		}else {
			showdialog('dialogmanual', 0, 'Vui lòng chọn sản phẩm', '', '');
			return;
	...
	$("#btnvisible")
			.click(
					function() {
						...
						showdialogconfirmfunc(
								'dialogmanual',
								'Bạn có muốn hiện sản phẩm không?',
								function() {
									if (c != null) {
		...
		load_image_by_id(c, function(out) {
			if (out == true) {
				...
				$('#dlgimageproduct').dialog({
					autoOpen : false,
					title : 'Danh sách hình của sản phẩm: '+c,
					width : 920,
					height : 600,
	...
	$("#btnremoveallimg").click(
			function() {
				...
				if (str.length == 0) {
					showdialog('dialogmanual', 0, 'Vui lòng chọn hình để xóa!', '',
							'');
					return;
				}
				var pdata = {
