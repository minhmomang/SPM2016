...
$(function(){
	var total = 0;
	var columns_data = [];
	var item_income_report = "<tr><td>@type</td><td style='float:right;margin-right:25px;'>@income</td></tr>";
		...
		Return_get("OrderController","get_info_chart_report",{"pdate":customdate},"GET",function(data){
			if(data!=null){
				...
				for(var i=0;i<data.length;i++){
					...
					str=str.replace('@income',formatcurrency(parseInt(data[i].total)));
					html=html+str;
					total = total+ parseInt(data[i].total);
				}
			}
			var temp_total = item_total.replace('@total',formatcurrency(total));
	...
	function load_text(callback){
		var url=ReturnHostingCMS()+'js/report/byday/admin_report.txt';
