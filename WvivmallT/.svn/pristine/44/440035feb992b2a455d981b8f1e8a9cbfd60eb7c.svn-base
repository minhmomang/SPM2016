...
public class CategoryPromotionDAL {
	...
	public static void main(String[] args) {
		System.out.println(new Gson().toJson(get_list_productpromotion("B2",0,10)));
	}
	...
	public static ArrayList<CategoryPromotionModel> get_promotion_parent()
	{
		...
		try
		{
				String 	query="SELECT category_promotion_id,name,type,value_discount,ngay_ap_dung,ngay_ket_thuc FROM tb_category_promotion where isdelete = false and isvisible=true and  parent = '0';";
				List<Map<String, Object>> listob = new ArrayList<Map<String, Object>>();
				IConnectEJBS con = new IConnectEJBS();
				listob=con.GetDataReturnResultSet(query);
				for(int i=0;i<listob.size();i++){
	...
	public static ArrayList<CategoryPromotionModel> get_promotion_sub(String cate)
	{
		...
		try
		{
				String 	query="SELECT category_promotion_id,name,type,value_discount,ngay_ap_dung,ngay_ket_thuc,isvisible,date_create FROM tb_category_promotion where isdelete = false and  parent != '0' and isdelete=false and parent='"+cate+"';";
				List<Map<String, Object>> listob = new ArrayList<Map<String, Object>>();
				IConnectEJBS con = new IConnectEJBS();
				listob=con.GetDataReturnResultSet(query);
				for(int i=0;i<listob.size();i++){
